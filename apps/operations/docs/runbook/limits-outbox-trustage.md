...
The operations service exposes `POST /admin/limits-outbox/drain` for the platform
...
```yaml
name: operations_limits_outbox_drain
description: Drain pending Commit/Release rows from operations's limits_outbox
trigger:
...
- The endpoint returns 503 if the outbox worker is misconfigured (nil).
  Trustage retries per the on_failure block; the next operations deployment
...
1. Deploy operations with `LIMITS_GATE_ENABLED_OPERATIONS_TRANSFER=false`
...
3. Flip `LIMITS_GATE_ENABLED_OPERATIONS_TRANSFER=true` per the
