...
func NewFormTemplateSave(_ context.Context, repo repository.FormTemplateRepository) *FormTemplateSave {
	return &FormTemplateSave{repo: repo}
}
...
func (e *FormTemplateSave) Execute(ctx context.Context, payload any) error {
	...
	existing, getErr := e.repo.GetByID(ctx, ft.GetID())
	if getErr == nil && existing != nil {
		if _, err := e.repo.Update(ctx, ft); err != nil {
	...
	if err := e.repo.Create(ctx, ft); err != nil && !data.ErrorIsDuplicateKey(err) {
