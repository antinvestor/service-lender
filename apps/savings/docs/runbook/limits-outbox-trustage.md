...
The savings service exposes `POST /admin/limits-outbox/drain` for the platform
...
```yaml
name: savings_limits_outbox_drain
description: Drain pending Commit/Release rows from savings's limits_outbox
trigger:
...
- The endpoint returns 503 if the outbox worker is misconfigured (nil).
  Trustage retries per the on_failure block; the next savings deployment
...
1. Deploy savings with `LIMITS_GATE_ENABLED_SAVINGS_DEPOSIT=false` and
   `LIMITS_GATE_ENABLED_SAVINGS_WITHDRAWAL=false` and the new endpoint
   live (no traffic yet).
2. Register the Trustage workflow above. Verify it calls the endpoint
   and gets `{drained: 0}` consistently.
3. Flip `LIMITS_GATE_ENABLED_SAVINGS_DEPOSIT=true` per the
   shadow-then-enforce rollout (Plan 7).
4. Once deposit is stable, flip `LIMITS_GATE_ENABLED_SAVINGS_WITHDRAWAL=true`.
5. Watch the outbox table for accumulating non-pending rows
