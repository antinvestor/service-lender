...
///
/// After all forms are filled the agent sees a review screen and can submit.
class ApplicationFormWizard extends ConsumerStatefulWidget {
  ...
  /// Called when the agent presses "Submit" on the final review step.
  final VoidCallback onAllFormsCompleted;

  @override
  ConsumerState<ApplicationFormWizard> createState() =>
      _ApplicationFormWizardState();
}

class _ApplicationFormWizardState extends ConsumerState<ApplicationFormWizard> {
  late List<ProductFormRequirement> _applicationForms;

  /// Index into [_applicationForms] + 1 extra step for "Review".
  int _currentStep = 0;

  /// Template IDs for which a submission has been saved.
  ...
  void initState() {
    super.initState();
    _applicationForms = widget.product.requiredForms
        .where((r) => r.stage.toLowerCase() == 'application')
        .toList()
      ..sort((a, b) => a.order.compareTo(b.order));
  }

  int get _totalSteps => _applicationForms.length + 1; // +1 for Review

  bool get _isReviewStep => _currentStep == _applicationForms.length;

  String _stepLabel(int index) {
    if (index < _applicationForms.length) {
      final req = _applicationForms[index];
  ...
  void _goToStep(int index) {
    ...
    // Only allow jumping to completed forms or the next uncompleted one.
    if (index <= _currentStep ||
        _completedTemplateIds
            .contains(_applicationForms[_currentStep].templateId)) {
      ...
      children: [
          ...
          child: SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final isCompleted = i < _applicationForms.length &&
                    _completedTemplateIds
                        .contains(_applicationForms[i].templateId);
                final isReview = i == _applicationForms.length;
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text(
                '${_completedTemplateIds.length} of ${_applicationForms.length} forms completed',
  ...
  bool _canAdvance() {
    if (_currentStep >= _applicationForms.length) return false;
    final req = _applicationForms[_currentStep];
    return _completedTemplateIds.contains(req.templateId);
  }

  bool _allRequiredCompleted() {
    for (final req in _applicationForms) {
  ...
  Future<void> _saveSubmission(
    ...
    try {
      ...
      if (fileData.isNotEmpty) {
        // Store file references as string IDs in the fileRefs struct.
  ...
  Widget _buildReviewStep() {
    ...
    return ListView.builder(
      ...
      itemCount: _applicationForms.length,
      itemBuilder: (context, i) {
        final req = _applicationForms[i];
        final isCompleted = _completedTemplateIds.contains(req.templateId);
        final templateAsync =
