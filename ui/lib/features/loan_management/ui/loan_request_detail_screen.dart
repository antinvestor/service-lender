...
class LoanRequestDetailScreen extends ConsumerWidget {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _infoRow('Requested Amount', Text(formatMoney(request.requestedAmount))),
          _infoRow('Requested Term', Text('${request.requestedTermDays} days')),
          if (request.approvedAmount.units > 0 || request.approvedAmount.nanos > 0)
            _infoRow('Approved Amount', Text(formatMoney(request.approvedAmount))),
          if (request.approvedTermDays > 0)
            _infoRow('Approved Term', Text('${request.approvedTermDays} days')),
          ...
          if (request.submittedAt.isNotEmpty) _infoRow('Submitted', Text(request.submittedAt)),
          if (request.decidedAt.isNotEmpty) _infoRow('Decided', Text(request.decidedAt)),
          ...
          const SizedBox(height: 24),
          if (request.status == LoanRequestStatus.LOAN_REQUEST_STATUS_SUBMITTED)
  ...
  Widget _buildActions(BuildContext context, WidgetRef ref, LoanRequestObject request) {
    return Row(
      children: [
        Expanded(
  ...
  Future<void> _approve(BuildContext context, WidgetRef ref, String id) async {
    try {
      await ref.read(loanRequestProvider.notifier).approve(id);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Loan request approved')));
      }
    } catch (e) {
      if (context.mounted) {
  ...
  Future<void> _reject(BuildContext context, WidgetRef ref, String id) async {
    ...
    try {
      await ref.read(loanRequestProvider.notifier).reject(id, reason);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Loan request rejected')));
      }
    } catch (e) {
      if (context.mounted) {
