...
import '../providers/disbursement_providers.dart';
...
import '../providers/restructure_providers.dart';
import '../providers/schedule_providers.dart';
import '../widgets/loan_status_badge.dart';

/// Generates a simple idempotency key.
...
class LoanAccountDetailScreen extends ConsumerStatefulWidget {
  ...
  /// Whether the current user can perform management actions (disburse, restructure, etc.).
  /// Host app should set this based on its own authorization logic.
...
class _LoanAccountDetailScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final loanAsync = ref.watch(loanAccountDetailProvider(widget.loanId));
    final balanceAsync =
        ref.watch(loanBalanceDetailProvider(widget.loanId));
      ...
      error: (error, _) => Center(
        child: Column(
          ...
          children: [
            ...
            FilledButton.tonal(
              onPressed: () =>
                  ref.invalidate(loanAccountDetailProvider(widget.loanId)),
          ...
          Padding(
            ...
            child: balanceAsync.when(
              ...
              data: (balance) => _BalanceCard(
                balance: balance,
                currencyCode: loanMoneyCurrency(loan.principalAmount),
              ),
            ),
          ),
      ...
      builder: (dialogContext) => _DisbursementFormDialog(
        ...
        onSave: (channel, recipientReference) async {
          try {
            ...
            await ref
                .read(disbursementNotifierProvider.notifier)
                .create(
                  loanAccountId: loan.id,
                  channel: channel,
            ...
            ref.invalidate(loanAccountDetailProvider(widget.loanId));
            ref.invalidate(
              loanBalanceDetailProvider(widget.loanId),
            );
            if (context.mounted) {
  ...
  void _showRecordPaymentDialog(
    ...
    String? outstanding;
    if (balance != null) {
      outstanding = loanMoneyToAmountString(balance.totalOutstanding);
    }
    showDialog<void>(
      context: context,
      builder: (dialogContext) => _RecordPaymentFormDialog(
        ...
        onSave: (amount, paymentReference, channel, payerReference) async {
          try {
            ...
            ref.invalidate(repaymentListProvider(widget.loanId));
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
              ...
              children: [
                Expanded(
                  child: _BalanceItem(
                    ...
                    value: formatLoanMoney(balance.principalOutstanding),
                  ),
                ),
                Expanded(
                  child: _BalanceItem(
                    label: 'Interest Accrued',
                    value: formatLoanMoney(balance.interestAccrued),
                  ),
                ),
                Expanded(
                  child: _BalanceItem(
                    label: 'Fees Outstanding',
                    value: formatLoanMoney(balance.feesOutstanding),
                  ),
                ),
              ],
            ),
              ...
              children: [
                Expanded(
                  child: _BalanceItem(
                    ...
                    value: formatLoanMoney(balance.penaltiesOutstanding),
                  ),
                ),
                Expanded(
                  child: _BalanceItem(
                    label: 'Total Outstanding',
                    value: formatLoanMoney(balance.totalOutstanding),
                    isBold: true,
                  ),
                ),
                Expanded(
                  child: _BalanceItem(
                    label: 'Total Paid',
                    value: formatLoanMoney(balance.totalPaid),
                    isBold: true,
                    color: Colors.green,
                  ),
                ),
...
class _ScheduleTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final scheduleAsync = ref.watch(
      repaymentScheduleDetailProvider(loanAccountId),
    );
              ...
              rows: schedule.entries.map((entry) {
                return DataRow(
                  ...
                  cells: [
                    ...
                    DataCell(Text(formatLoanMoney(entry.principalDue))),
                    DataCell(Text(formatLoanMoney(entry.interestDue))),
                    DataCell(Text(formatLoanMoney(entry.totalDue))),
                    DataCell(Text(formatLoanMoney(entry.totalPaid))),
                    DataCell(Text(formatLoanMoney(entry.outstanding))),
...
class _TransactionsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final disbursementsAsync = ref.watch(
      disbursementListProvider(loanAccountId),
    );
    final repaymentsAsync = ref.watch(
      repaymentListProvider(loanAccountId),
    );
    ...
    for (final d in disbursements) {
      items.add(
        _TransactionItem(
          date: d.disbursedAt,
          type: 'Disbursement',
          amount: loanMoneyToAmountString(d.amount),
          currency: loanMoneyCurrency(d.amount),
    ...
    for (final r in repayments) {
      items.add(
        _TransactionItem(
          date: r.receivedAt,
          type: 'Repayment',
          amount: loanMoneyToAmountString(r.amount),
          currency: loanMoneyCurrency(r.amount),
...
class _PenaltiesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final penaltiesAsync = ref.watch(
      penaltyListProvider(loanAccountId),
    );
              ...
              child: ListTile(
                ...
                title: Text(
                  ...
                  style: theme.textTheme.titleSmall?.copyWith(
                    fontWeight: FontWeight.w600,
                    decoration:
                        penalty.isWaived ? TextDecoration.lineThrough : null,
                  ),
                ),
                subtitle: Text(
                  penalty.isWaived
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                try {
                  await ref
                      .read(penaltyNotifierProvider.notifier)
                      .waive(id: penalty.id, reason: reason);
                ...
                itemBuilder: (context, index) {
                  ...
                  return _RestructureCard(
                    ...
                    onApprove: () async {
                      try {
                        await ref
                            .read(restructureNotifierProvider.notifier)
                            .approve(r.id);
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: DropdownButtonFormField<RestructureType>(
                    ...
                    items: RestructureType.values
                        .where(
                          (t) =>
                              t !=
                              RestructureType.RESTRUCTURE_TYPE_UNSPECIFIED,
                        )
                        .map(
                          (t) => DropdownMenuItem(
                            value: t,
                ...
                FormFieldCard(
                  ...
                  description:
                      'The revised annual interest rate for the loan.',
                  child: TextField(
                ...
                FormFieldCard(
                  ...
                  child: TextField(
                    ...
                    decoration:
                        const InputDecoration(hintText: 'e.g. 365'),
                ...
                FormFieldCard(
                  label: 'Reason',
                  description:
                      'Why this restructuring is being requested.',
                  isRequired: true,
                  child: TextField(
                    controller: reasonCtrl,
                    decoration:
                      ...
                      try {
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content:
                                  Text('Restructure request created'),
                            ),
                          );
                        }
                      } catch (e) {
          ...
          child: Column(
            ...
            children: [
              ...
              FormFieldCard(
                label: 'Amount',
                description:
                    'The repayment amount received from the borrower.',
                isRequired: true,
                child: TextFormField(
              ...
              FormFieldCard(
                ...
                child: TextFormField(
                  ...
                  decoration:
                      const InputDecoration(hintText: 'e.g. TXN-12345'),
                  textInputAction: TextInputAction.next,
              ...
              FormFieldCard(
                ...
                description:
                    'Identifier for the person making the payment.',
                isRequired: true,
                child: TextFormField(
